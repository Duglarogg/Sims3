...
namespace Duglarogg.AbductorSpace.Helpers
{
    ...
    {
        static ResourceKey[] kAlienSkinTones = new ResourceKey[]
            {
        ...
        static Color[] kAlienHairColors = new Color[]
            {
        ...
        static AlienGenetics()
        { }

        public static float GetSkinToneIndex(ResourceKey tone)
        {
            float result;

            if (tone == kAlienSkinTones[0])
            {
                result = 1f;
            }
            else if (tone == kAlienSkinTones[2])
            {
                result = 0.5f + (0.5f * RandomUtil.GetFloat(1f));
            }
            else if (tone == kAlienSkinTones[5])
            {
